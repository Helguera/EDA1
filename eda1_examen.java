 ...
 * @version Aqui se realizaran las modificaciones que serviran de base para la
 *          practica 2.
...
public class eda1_examen {
	...
	public static int vPI[] = { 71, 241, 180, 230, 11, 106, 114, 72, 133, 78, 158, 235, 226, 248, 148, 83, 224, 187,
	...
	public static int vPS[] = { 20, 83, 15, 86, 179, 200, 122, 156, 235, 101, 72, 23, 22, 21, 159, 2, 204, 84, 124, 131,
	...
	public static void main(String[] args) {
		...
		int evil_vector[] = { 101, 118, 105, 108, 046, 99, 111, 114, 112, 064, 109, 97, 100, 046, 111, 114,
				103 };
		...
		int longitud_fich = (int) fichero_l.length(); // l_fich mide la longitud
														// del fichero
		...
		boolean encontrado = false; // Variable para ver si se ha encontrado el
									// texto
		/**
		 * Se abre el fichero en vector_fichero
		 */
		try (DataInputStream fichero = new DataInputStream(new FileInputStream(fichero_usuario))) {
			for (int contador = 0; contador != longitud_fich; contador++) {
				vector_fichero[contador] = fichero.readUnsignedByte();
			}
		} catch (
		...
		for (int clave = 0; clave < 65536; clave++) { // Bucle que recorra todas las claves
			...
			for (int i = 0; i < (longitud_fich); i++) { // Bucle que recorre todas las posiciones del vector a buscar
				...
				if (cont_evil == longitud_evil - 1) {
					...
					int clave_inicio = clave - longitud_evil - 100;
